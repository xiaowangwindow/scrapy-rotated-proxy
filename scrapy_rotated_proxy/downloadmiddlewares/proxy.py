...
class RotatedProxy(object):
    ...
    def from_crawler(cls, crawler):
        ...
        crawler.signals.connect(o.proxy_block_received, signal=proxy_signals.proxy_block)
        return o
    ...
    def proxy_block_received(self, spider, response, exception):
        if response.meta.get('proxy'):
            ...
            creds = response.request.headers.get('Proxy-Authorization').split(b' ')[-1] \
                if response.request.headers.get('Proxy-Authorization') else None
            self.black_proxies[scheme].add((creds, response.meta.get('proxy')))
            logger.debug('Block proxy: {proxy}, Total block {count} {scheme} proxy'.format(
    ...
    def process_request(self, request, spider):
        if not self.proxies:
            ...
            for scheme, proxies in self.proxies.items():
                logger.debug('Loaded {count} {scheme} proxy from {origin}'.format(
                    count=len(proxies),
                    scheme=scheme,
                    origin=self.proxies_storage.__class__.__name__
                ))

        if 'proxy' in request.meta and not request.dont_filter:  # When Retry, dont_filter=True, reset proxy
    ...
    def _set_proxy(self, request, scheme):
        creds, proxy = next(self.proxy_cycle(scheme))
        print(proxy)
        request.meta['proxy'] = proxy
        if creds:
            request.headers['Proxy-Authorization'] = b'Basic ' + creds

    def proxy_cycle(self, scheme):
        if not self.proxy_gen.get(scheme):
            self.proxy_gen[scheme] = self.proxy_generator(scheme)
        return self.proxy_gen[scheme]

    def proxy_generator(self, scheme):
        while True:
            self.proxies[scheme] -= self.black_proxies.get(scheme, set())
            if not self.proxies[scheme]:
                # TODO close spider
                logger.info('Run out of all {scheme} proxies'.format(scheme=scheme))
